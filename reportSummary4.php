...
<table border="0" cellspacing="0" cellpadding="0" width="750">
	<tr>
		...
		?> 
		Transaction Summary as of <?php echo $mysqldate;?>
...
<tr>
	<th colspan="4" align="left">Summary</th>
				...
				<tr>
					...
					$checkPaids1 = dbquery("SELECT count(bill_no) as counter, bill_desc, sum(ass_amount) as paidTotal FROM bill_receipt INNER JOIN bill_assessment ON receipt_no=ass_invoice_no INNER JOIN  bill_bills ON ass_bill_no=bill_no WHERE (receipt_user='".$_GET['user_no']."' and receipt_sy='".$_GET['enrol_sy']."' AND bill_category='".$dataBills['bill_category']."' and ass_sy='".$current_sy."') GROUP BY bill_desc");
					...
					<td align="right">
					<?php 
					$checkPaids = dbquery("SELECT SUM(receipt_amtPaid) as totalPaid FROM bill_receipt INNER JOIN bill_assessment ON receipt_no=ass_invoice_no INNER JOIN  bill_bills ON ass_bill_no=bill_no WHERE (receipt_user='".$_GET['user_no']."' and receipt_sy='".$_GET['enrol_sy']."' AND bill_category='".$dataBills['bill_category']."' and ass_sy='".$current_sy."') ORDER BY receipt_datetime DESC");
...
<tr>
<?php
$result= dbquery("SELECT * FROM bill_receipt WHERE (receipt_user='".$_GET['user_no']."' and receipt_sy='".$_GET['enrol_sy']."') ORDER BY receipt_datetime DESC");
$rows = dbrows($result);
?>
<table border="1" cellspacing="0" cellpadding="1" width="750">
	<tr>
