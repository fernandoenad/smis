...
<table border="1" cellspacing="0" cellpadding="1" width="1500">
	...
	<?php
		...
		$checkCourses = dbquery("SELECT * FROM class INNER JOIN prospectus ON class.class_pros_no=prospectus.pros_no WHERE (class.class_sy='".$_GET['enrol_sy']."' AND class.class_section_no='".$dataSection['section_no']."' and pros_sem='".$active_sem."') ORDER BY pros_sort ASC");
	...
	<tr>
		...
		?>
		<td align="center" bgcolor="yellow"><strong><?php echo ($countUnits!=$gradedUnits1 || $countUnits!=$gradedUnits2 || $countUnits!=$gradedUnits3 || $countUnits!=$gradedUnits4 || $countUnits!=$gradedUnitsqf?"":($dataGrade1['grade_final']<75?"<font color=red>".$dataGrade1['grade_final']."</font>":$dataGrade1['grade_final']));?></td>
		<?php
		} 
		?>
		<?php 
		...
		<td align="center" bgcolor="yellow"><b><?php echo ($countUnits!=$gradedUnits1 || $countUnits!=$gradedUnits2 || $countUnits!=$gradedUnits3 || $countUnits!=$gradedUnits4 || $countUnits!=$gradedUnitsqf?"":number_format($aveQf/$countUnits,3));?></b></td>
		<td align="center" bgcolor="yellow"><b><?php echo ($countUnits!=$gradedUnits1 || $countUnits!=$gradedUnits2 || $countUnits!=$gradedUnits3 || $countUnits!=$gradedUnits4 || $countUnits!=$gradedUnitsqf?"":round($aveQf/$countUnits,0));?></b></td>
		<td align="left">
		<?php
		if($countUnits!=$gradedUnits1 || $countUnits!=$gradedUnits2 || $countUnits!=$gradedUnits3 || $countUnits!=$gradedUnits4 || $countUnits!=$gradedUnitsqf){
		}
		else{
			if(round($aveQf/$countUnits,0)>=98 && $below80counter==0){
				echo "<b>Hghst Honor";
			}
			else if(round($aveQf/$countUnits,0)>=95 && $below80counter==0){
				echo "<b>High Honor";
			}
			else if(round($aveQf/$countUnits,0)>=90 && $below80counter==0){
				echo "<b>w/ Honor";
			}
	...
	<tr>
		<td><?php echo $i;?></td>
		<td><?php echo $data['stud_lname'];?>, <?php echo $data['stud_fname'];?> <?php echo substr($data['stud_mname'],0,1);?></td>
		<?php
		$countUnits=0;
		$gradedUnits1=0;
		$gradedUnits2=0;
