...
$checkStudent = dbquery("SELECT * FROM student INNER JOIN studenroll on student.stud_no=studenroll.enrol_stud_no WHERE (student.stud_no='".$_GET['stud_no']."' AND enrol_sy!='".$current_sy."') order by enrol_sy desc");
...
<div class="table-responsive">
	<table>
		...
		while($dataPayables = dbarray($checkPayables)){
		if(substr($dataPayables['pros_title'],0,3)!="***"){
		?>													
			<tr>
				<td><?php echo $dataPayables['bill_desc'];?></td>
				<td><?php echo $dataPayables['ass_amount'];?></td>
		...
		<tr>
			<td align="left"><b>Total Payables: <?php echo number_format($sumPayable,2);?> <br> Payments Made: <?php echo number_format($sumPaid,2);?></b></td>
