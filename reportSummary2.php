...
<table border="0" cellspacing="0" cellpadding="0" width="750">
	<tr>
		...
		?> 
		<h3>Transaction Report Summary for <?php echo $mysqldate;?>
		</h3>
		</td>
	</tr>
</table>	
...
<tr>
<?php
$result= dbquery("SELECT * FROM bill_receipt WHERE (DATE(`receipt_datetime`) BETWEEN '".$_GET['date']."' AND '".$_GET['date']."') ORDER BY receipt_user ASC, receipt_datetime DESC");
$rows = dbrows($result);
?>
<table border="1" cellspacing="0" cellpadding="1" width="750">
	<tr>
	...
	<tr>
		...
		<td>
		<?php 
			$checkUser = dbquery("SELECT * FROM users WHERE (user_no='".$data['receipt_user']."')");
			$dataUser = dbarray($checkUser);
