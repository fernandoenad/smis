...
<tr>
	<th colspan="4" align="left">Summary</th>
				...
				<tr>
					...
					<?php 
					$sumPaid = 0;
					$checkPaids = dbquery("SELECT SUM( ass_amount ) AS totalPaid,  `bill_no` ,  `bill_category` ,  `bill_desc`  FROM bill_bills INNER JOIN bill_assessment ON bill_no=ass_bill_no INNER JOIN bill_receipt ON ass_invoice_no=receipt_no WHERE (bill_no='".$_GET['bill_no']."' and bill_sy='".$_GET['enrol_sy']."' and receipt_sy='".$_GET['enrol_sy']."') GROUP BY bill_desc ORDER BY bill_prio ASC");
...
<tr>
<?php
$result= dbquery("SELECT *  FROM bill_bills INNER JOIN bill_assessment ON bill_no=ass_bill_no INNER JOIN bill_receipt ON ass_invoice_no=receipt_no WHERE (bill_no='".$_GET['bill_no']."' and bill_sy='".$_GET['enrol_sy']."' and receipt_sy='".$_GET['enrol_sy']."') order by receipt_datetime desc");
$rows = dbrows($result);
?>
<table border="1" cellspacing="0" cellpadding="1" width="750">
	<tr>
	...
	<tr>
		...
		<td><?php 
			$checkItems = dbquery("SELECT * FROM bill_ledger INNER JOIN bill_assessment ON ledger_ass_no=ass_no INNER JOIN bill_bills ON ass_bill_no=bill_no WHERE (ledger_receipt_no='".$data['receipt_no']."' AND bill_no='".$_GET['bill_no']."')");
		...
		<?php
		// $checkReceipt = dbquery("select * from bill_receipt where  ");
		?>
		<td align="right"><?php echo number_format($data['ass_amount'],2); ?> <?php echo ($data['receipt_amtPaid']==0?" (voided)":""); ?></td>
	</tr>
	<?php 
	$sumPaid=$sumPaid + $data['ass_amount'];
	$i++;
	} ?>
	<tr>
